...
class YellowDepthDetector(Node):
    def __init__(self):
        ...
        self.cx=0.00
        self.cy=0.00
        self.centroid_is_not_yellow = False
        ...
        if len(contours)!=0:
            ...
            if ((w > self.threshold or h > self.threshold)):
                ...
                if mask[int(self.cy), int(self.cx)] == 0:
                    self.centroid_is_not_yellow = True
                else:
                    self.centroid_is_not_yellow = False

                if (self.centroid_is_not_yellow):
                    ...
                    if corner_list:
                        ...
                        self.is_yellow_corner_dist = corner_z
                    cv2.circle(frame,(int(self.cx), int(self.cy)), 8, (255, 0, 255), 3)
                else:
                    self.is_yellow_corner_detected = False
                    self.is_yellow_corner_dist = 100.00
        
        cv2.imshow("frame", frame)
        cv2.imshow("mask2", mask2)
        cv2.imshow("mask", mask)
        cv2.waitKey(1)
        self.is_corner_detected_pub.publish(self.is_yellow_corner_detected)
        self.corner_distance_pub.publish(self.yellow_corner_dist_msg)
